...
-- Fix the record assignment issue in generate_workshop_public_link function
...
BEGIN
  ...
  IF FOUND THEN
    ...
    RETURN QUERY SELECT 
      existing_link_id as link_id,
      existing_link_slug as slug,
      ('/book/' || existing_link_slug) as public_url;
    RETURN;
  END IF;
  ...
  -- Return new link
  RETURN QUERY SELECT 
    new_link_id as link_id,
    new_link_slug as slug,
    ('/book/' || new_link_slug) as public_url;
END;
$$;
